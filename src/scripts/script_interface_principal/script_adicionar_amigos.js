...
async function renderUsers(users) {
    ...
    for (const user of users) {
        ...
        if (!alreadyDisplayed) {
            ...
            userFoto.src = `http://localhost:3000/api/foto/${user._id}`
...
async function checkIfFriend(userId) {
    const idUsuarioPesquisado = localStorage.getItem('usuarioPesquisado');
    try {
        const response = await fetch(`http://localhost:3000/api/checkAmigo?userId=${idUsuarioPesquisado}&friendId=${userId}`);
        if (!response.ok) {
